		...
		bitmap_t** bm_g, bitmap_t** bm_o, bitmap_t** bm_s, int be)
{
	...
	*bm_g = *bm_o = *bm_s = 0;
	...
	if (be) {
		blur((*bm_g)->buffer, priv->tmp, (*bm_g)->w, (*bm_g)->h, (*bm_g)->w, (unsigned int*)priv->gt2, priv->g_r, priv->g_w);
		if (*bm_o)
			blur((*bm_o)->buffer, priv->tmp, (*bm_o)->w, (*bm_o)->h, (*bm_o)->w, (unsigned int*)priv->gt2, priv->g_r, priv->g_w);
	}
