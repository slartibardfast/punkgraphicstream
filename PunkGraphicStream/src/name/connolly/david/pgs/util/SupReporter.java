    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            for (int nextEnd = 2; nextEnd < packets.size(); nextEnd++) {
                BigDecimal firstTimecode;
                BigDecimal secondTimecode;
                ...
                pw.println("No: " + count + " ");
                text = Integer.toString(buf[start + 10] & 0xFF, 16).toUpperCase();
                if (text.length() == 1) {
                    text = "0" + text;
                }
                pw.print("Type: " + text + " ");
                ...
                firstTimecode = new BigDecimal(new BigInteger(unsignedLongTimecode));
                firstTimecode = firstTimecode.divide(BigDecimal.valueOf(90), 3, RoundingMode.DOWN);
                pw.print("Timecode: " + firstTimecode + "ms - ");
                for (int i = 1, j = start + 6; j < start + 10; i++, j++) {
                    unsignedLongTimecode[i] = buf[j];
                }
                secondTimecode = new BigDecimal(new BigInteger(unsignedLongTimecode));
                secondTimecode = secondTimecode.divide(BigDecimal.valueOf(90), 3, RoundingMode.DOWN);
                pw.print(secondTimecode + "ms ");
                if (secondTimecode.compareTo(BigDecimal.valueOf(0.0001)) > 0) {
                    pw.print("Duration?: " + firstTimecode.subtract(secondTimecode) + "ms ");
                }
